function h = corr_colormaps_STD(anat_bkg_path,r_map_GEASL,rPearson_GEASL,outpath,figname,L)
...
for ss = 1 : length(slices)
    ...
    h.Color = [0 0 0];
     saveas(h,fullfile(outpath,strcat('FigSTD',figname,'_',num2str(ss))),'fig');
    exportgraphics(h,fullfile(outpath,strcat('FigSTD',figname,'_',num2str(ss),'.png')),'Resolution',300)
    if ss == length(slices)
        c = colorbar;
        ...
        exportgraphics(h,fullfile(outpath,strcat('FigSTD',figname,'_colorbar.png')),'Resolution',300)
         close(h);
    else
         close(h);
    end
...
h.Color = [0 0 0];
saveas(h,fullfile(outpath,strcat('FigSTDsag',figname)),'fig');
exportgraphics(h,fullfile(outpath,strcat('FigSTDsag',figname,'.png')),'Resolution',300)
close(h);
...
h.Position = [1 1 1000 800];
h.Color = [0 0 0];
saveas(h,fullfile(outpath,strcat('FigSTDcor',figname)),'fig');
exportgraphics(h,fullfile(outpath,strcat('FigSTDcor',figname,'.png')),'Resolution',300)
