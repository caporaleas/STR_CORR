function h = corr_colormaps_pval(anat_bkg_path,r_map_GEASL,p_map_GEASL,rPearson_GEASL,outpath,figname,L)
...
for ss = 1 : length(slices)
    ...
    h.Color = [0 0 0];
    saveas(h,fullfile(outpath,strcat('FigP',figname,'_',num2str(ss))),'fig');
    exportgraphics(h,fullfile(outpath,strcat('FigP',figname,'_',num2str(ss),'.png')),'Resolution',300)
    if ss == length(slices)
        c = colorbar;
        c.Color = [1 1 1]; c.FontSize = 16; c.TickLength = 0; c.Position = [0.90 0.27 0.0144 0.3];
        saveas(h,fullfile(outpath,strcat('FigP',figname,'_colorbar')),'fig');
        exportgraphics(h,fullfile(outpath,strcat('FigP',figname,'_colorbar.png')),'Resolution',300)
        close(h);
    else
        close(h);
    end
...
h.Color = [0 0 0];
saveas(h,fullfile(outpath,strcat('FigPsag',figname)),'fig');
exportgraphics(h,fullfile(outpath,strcat('FigPsag',figname,'.png')),'Resolution',300)
close(h);
...
h.Position = [1 1 1000 800];
h.Color = [0 0 0];
saveas(h,fullfile(outpath,strcat('FigPcor',figname)),'fig');
exportgraphics(h,fullfile(outpath,strcat('FigPcor',figname,'.png')),'Resolution',300)
