function [median_R1_GM,median_R1_subGM,median_R1_WM,median_diff_GM,median_diff_subGM,median_diff_WM,GM_ROI_vol,subGM_ROI_vol,WM_ROI_vol] = STR_CORR_08tris_medians(initpath,parameters,V_R1_maps_filtered,V_diff_maps_filtered,PVE_masks,AtlasGM_masks_filtered,AtlassubGM_masks_filtered,AtlasWM_masks_filtered)
...
for ii = 1 : N_subj 
    ...
    for rr = 1 : N_subGM_ROIs
        ...
        tmp2 = V_R1_maps_filtered{ii,1}.*PVE_masks{ii,2}.*AtlassubGM_masks_filtered{rr,1};
        V_R1_masked_arr1 = tmp(tmp~=0);
        V_R1_masked_arr2 = tmp2(tmp2~=0);
        V_R1_masked_arr = cat(1,V_R1_masked_arr1,V_R1_masked_arr2);
    ...
    for pp = 1 : N_par-1
        ...
        for rr = 1 : N_subGM_ROIs
            ...
            tmp2 = V_diff_maps_filtered{ii,pp}.*PVE_masks{ii,2}.*AtlassubGM_masks_filtered{rr,1};
            V_diff_masked_arr1=tmp(tmp~=0);
            V_diff_masked_arr2=tmp2(tmp2~=0);
            V_diff_masked_arr = cat(1,V_diff_masked_arr1,V_diff_masked_arr2);
