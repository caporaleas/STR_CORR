function [corr_acrossROIs_noout_GM,corr_acrossROIs_noout_WM] = STR_CORR_11tris_corr_median_medians_acrossROIs_noout(outlierremoval,median_R1_GM,median_R1_subGM,median_R1_WM,median_diff_GM,median_diff_subGM,median_diff_WM,parameters,outpath,confidence_bounds)
...
if outlierremoval == 1
    plotname = 'Scatterplot_WM_GM_AND_SUBGM_diff_vs_R1_acrossROIs_noout';
else
    plotname = 'Scatterplot_WM_GM_AND_SUBGM_diff_vs_R1_acrossROIs';
end
...
median_median_diff_WM2 = zeros(size(median_median_diff_WM));

% outliers considering GM ROIs + subGM ROIs + WM ROIs (cumulative)
if outlierremoval == 1
    for pp = 1 : N_par-1
        tmp1 = median_median_diff_GM(1,:,pp);
        tmp2 = median_median_diff_subGM(1,:,pp);
        tmp3 = median_median_diff_WM(1,:,pp);
        yy = cat(2,tmp1,tmp2,tmp3);
        tmp = yy;
        ...
        median_median_diff_subGM2(1,:,pp) = yy(length(tmp1)+1:length(tmp1)+length(tmp2));
        median_median_diff_WM2(1,:,pp) = yy(length(tmp1)+length(tmp2)+1:end);
    end

    median_median_diff_GM = median_median_diff_GM2;
    median_median_diff_subGM = median_median_diff_subGM2;
    median_median_diff_WM = median_median_diff_WM2;
end
% outliers considering GM ROIs, subGM ROIs, WM ROIs in each group
%{
if outlierremoval == 1
    for pp = 1 : N_par-1
...
for pp = 1 : N_par-1
    ...
    subplot(2,4,pp) 
    xx = cat(2,median_median_R1_GM,median_median_R1_subGM,median_median_R1_WM);
    yy = cat(2,median_median_diff_GM(1,:,pp),median_median_diff_subGM(1,:,pp),median_median_diff_WM(1,:,pp));
    ...
    if confidence_bounds == 1
        ...
        plot(x_fit, y_fit, 'b-', 'LineWidth', 2) % thick black line
        hold on
        %plot(xv, y_ext, '-b')
    ...
    title({strcat('r=',corr_coef_str_GM,'; p=',p_value_str_GM)});
